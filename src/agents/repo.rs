...
impl Repo {
    fn start_task(&mut self, req: Request) {
        ...
        let idb_open_request: web_sys::IdbOpenDbRequest = idb_factory.open_with_u32("more-podcasts", 1).unwrap();
        ...
        let closure_update = Closure::wrap(Box::new(move |event: web_sys::Event| {
...
impl Agent for Repo {
    ...
    fn update(&mut self, msg: Self::Message) {
        match msg {
            Msg::OpenDbUpdate(e) => {
                log::info!("update {:?}", e);
                let idb_db = IdbDatabase::from(self.open_request.as_ref().unwrap().request.result().unwrap());
                log::info!("db: {:?}", idb_db);
